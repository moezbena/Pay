...
function validateAndInsert(theform) {
	theform.action.value="<%=STATUT_QUALITE.ACTION_INSERT%>";
	theform.submit(); 
}
function onlySelect(theform) {
	theform.action.value="<%=STATUT_QUALITE.ACTION_SELECT%>";
	theform.submit(); 
}
function validateAndUpdate(theform) {
	theform.action.value="<%=STATUT_QUALITE.ACTION_UPDATE%>";
	theform.submit();    
}
function confirmAndDelete(theform) {
	if(confirm('Voulez-vous vraiment supprimer cet enregistrement ?')) {
		theform.action.value="<%=STATUT_QUALITE.ACTION_DELETE%>";
